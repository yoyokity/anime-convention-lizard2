...
function apply(ctx, config) {
  ...
  const getChannelId = /* @__PURE__ */ __name((session) => {
    return session.guildId ? session.channelId : `private:${session.userId}`;
  }, "getChannelId");
  const conventionCmd = ctx.command("漫展", "漫展查询和订阅管理");
  conventionCmd.subcommand(".查询 <keyword>", "查询指定城市或主题的漫展信息").action(async ({ session }, keyword) => {
    if (!keyword) {
    ...
    try {
      ...
      const selectionMessage = response.data.map((item, index) => `${index + 1}. ${item.name}
   - ${item.address}`).join("\n");
...
请输入对应的序号查看详细信息，输入“0”取消操作。`);
  ...
  conventionCmd.subcommand(".一键查询", "查询所有订阅关键词的漫展信息").action(async ({ session }) => {
    ...
    for (const subscription of subscriptions) {
      ...
      try {
        ...
        if (response.code === 200 && response.data?.length > 0) {
          results.push(...response.data.map((item) => ({ ...item, keyword: subscription.keyword })));
        } else {
    ...
    if (results.length === 0) {
      await session.send(
        failedKeywords.length > 0 ? `未找到任何订阅关键词的相关漫展信息（失败关键词：${failedKeywords.join("，")}）。` : "未找到任何订阅的漫展信息。"
      );
      return;
    }
...
`;
  ...
  conventionCmd.subcommand(".订阅 <keyword>", "订阅指定城市或主题的漫展信息").action(async ({ session }, keyword) => {
    if (!keyword) {
  ...
  conventionCmd.subcommand(".取消订阅 [keyword]", "取消订阅指定城市或主题的漫展信息，若无关键词则取消所有订阅").action(async ({ session }, keyword) => {
    const channelId = getChannelId(session);
    const subscriptions = await ctx.database.get("anime_convention", {
      userId: session.userId,
      channelId
    ...
    if (!keyword) {
      ...
      const confirmation = await session.prompt(1e4);
  ...
  conventionCmd.subcommand(".订阅列表", "查看已经订阅的漫展关键词").action(async ({ session }) => {
    const channelId = getChannelId(session);
    const subscriptions = await ctx.database.get("anime_convention", {
      userId: session.userId,
      channelId
