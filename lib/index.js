...
var inject = ["database"];
var name = "anime-convention-lizard";
var usage = `
# 🎉 开箱即用的漫展查询插件
...
var Config = import_koishi.Schema.object({
  apiUrl: import_koishi.Schema.string().default("https://www.hhlqilongzhu.cn/api/manzhan_sou.php").description("默认API请勿更改")
});
function apply(ctx, config) {
  ...
  const getChannelId = /* @__PURE__ */ __name((session) => session.guildId ? session.channelId : `private:${session.userId}`, "getChannelId");
  ctx.command("漫展", "漫展查询和订阅管理").subcommand(".查询 <keyword>", "查询漫展").action(async ({ session }, keyword) => {
    if (!keyword) {
    ...
    try {
      ...
      const message = response.data.map((item, i) => `${i + 1}. ${item.name} - ${item.address}`).join("\n");
...
请输入序号查看详情，输入“0”取消。`);
  ...
  ctx.command("漫展", "漫展查询和订阅管理").subcommand(".一键查询", "查询订阅的所有漫展").action(async ({ session }) => {
    ...
    const results = await Promise.all(subscriptions.map(async (sub) => {
      try {
        ...
        return response.code === 200 ? response.data.map((item) => ({ ...item, keyword: sub.keyword })) : [];
      } catch {
        return [];
      }
    }));
...
请输入序号查看详情，输入“0”取消。`);
    ...
    userSearchCache[session.userId].timeoutId = setTimeout(() => delete userSearchCache[session.userId], 15e3);
  });
  ctx.command("漫展", "漫展查询和订阅管理").subcommand(".订阅 <keyword>", "订阅漫展").action(async ({ session }, keyword) => {
  ...
  ctx.command("漫展", "漫展查询和订阅管理").subcommand(".取消订阅 [keyword]", "取消订阅").action(async ({ session }, keyword) => {
    const channelId = getChannelId(session);
    if (!keyword) {
      await session.send("确定取消所有订阅？（是/否）");
      if ((await session.prompt(1e4))?.toLowerCase() === "是") {
  ...
  ctx.command("漫展", "漫展查询和订阅管理").subcommand(".订阅列表", "查看订阅列表").action(async ({ session }) => {
    const subscriptions = await ctx.database.get("anime_convention", { userId: session.userId, channelId: getChannelId(session) });
    if (!subscriptions.length) {
      await session.send("你没有订阅任何漫展。");
      return;
  ...
  ctx.middleware(async (session, next) => {
    ...
    const result = `漫展名称: ${selectedItem.name}
...
参与方式: ${selectedItem.isOnline ? "线上" : "线下"}`;
    try {
      ...
      await session.send(`${import_koishi.h.image(img)}
${result}`);
    } catch (error) {
